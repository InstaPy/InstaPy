...
def job():
    try:
        ...
        if is_not_hour_to_run.check_limit() is True:
            ...
            return 0
        taglist = ['6months', 'proudmommy', 'toddlerfashion','girlmom','bows','momofgirls', 'babygirl', 'mommylife', 'momanddaughter', 'baby', 'mybabygirl', 'babygirl']
        ...
        session.set_dont_like(['boy', 'boys', 'store'])
        #limits
        ...
        # unfollow
        session.unfollow_users(amount=100, onlyInstapyFollowed=True, onlyInstapyMethod='FIFO', sleep_delay=600)

        # start the process
        print("do the actual liking by tag")
        session.like_by_tags(amount=200, use_smart_hashtags=True)
