...
try:
    ...
    #todo: set words/tags to avoid
    #todo: unfollow method

    #todo: minimum posts
    #todo: avoid private accounts
    #todo: avoid interaction with private accounts that i do not follow

    session.set_dont_include(dont_include)
    session.set_dont_like(['pizza', 'girl','islænder','gay'])

    session.set_user_interact(amount=2, randomize=True, percentage=51, media='Photo')


    session.follow_likers(['salomon_danmark', 'northcoastultra', '9000running'], photos_grab_amount=2, follow_likers_per_photo=3, randomize=True,
                           sleep_delay=6, interact=False)

    my_followers = session.grab_followers(username="meinerttt", amount="full", live_match=False, store_locally=True)

    my_following = session.grab_following(username="meinerttt", amount="full", live_match=False,
                                                 store_locally=True)

    my_nonfollowers = session.pick_nonfollowers(username="meinerttt", live_match=False, store_locally=True)

    my_fans = session.pick_fans(username="meinerttt", live_match=False, store_locally=True)

    my_mutualFollowing = session.pick_mutual_following(username="meinerttt", live_match=False,
                                                           store_locally=True)

    # actions
    #session.like_by_tags(['løb'], amount=1)
