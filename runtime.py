...
logging.basicConfig(
    filename='statistics/logs/' + now + '-' + insta_username + '_instabot.log',
    level=logging.INFO)
...
try:
    ...
    session.set_do_comment(enabled=module.set_do_comment['enabled'],
                           percentage=module.set_do_comment['percentage'])
    session.set_do_follow(enabled=module.set_do_follow['enabled'],
                          percentage=module.set_do_follow['percentage'],
                          times=module.set_do_follow['times'])
    session.set_user_interact(amount=module.set_user_interact['amount'],
                              percentage=module.set_user_interact[
                                  'percentage'],
                              randomize=module.set_user_interact['randomize'],
                              media=module.set_user_interact['media'])
    session.set_dont_unfollow_active_users(
        enabled=module.set_dont_unfollow_active_users['enabled'],
        posts=module.set_dont_unfollow_active_users['posts'])
    session.set_relationship_bounds(
        enabled=module.set_relationship_bounds['enabled'],
        potency_ratio=module.set_relationship_bounds['potency_ratio'],
        delimit_by_numbers=module.set_relationship_bounds[
            'delimit_by_numbers'],
    ...
    session.set_delimit_liking(enabled=module.set_delimit_liking['enabled'],
                               min=module.set_delimit_liking['min'],
                               max=module.set_delimit_liking['max'])
    session.set_delimit_commenting(
        enabled=module.set_delimit_commenting['enabled'],
        min=module.set_delimit_commenting['min'],
    ...
    session.set_ignore_if_contains(
        module.set_ignore_if_contains['ignoreifcontainslist'])

    if module.set_dont_include['source'] == 'file':
        print('---> Dont include by file')
        friends = json.load(
            open(module.set_dont_include['dontincludefilename']))
        session.set_dont_include(friends)
        print('---> These friends are not included: ' + str(
            friends[0:5]) + ' and ' + str(len(friends) - 6) + ' more.')
    else:
        print('---> Dont include by list')
        session.set_dont_include(module.set_dont_include['dontincludelist'])
        # print('---> These friends are not included: ' + str(
        # module.set_dont_include['dontincludelist']))
    session.set_simulation(enabled=module.set_simulation['enabled'],
                           percentage=module.set_simulation['percentage'])

    ### LOAD ACTIONS ###
    if module.follow_by_list['enabled'] == True and type != 'only_unfollow':
        print('---> Starting following by list')
        session.follow_by_list(module.follow_by_list['followlist'],
                               times=module.follow_by_list['times'],
                               sleep_delay=module.follow_by_list[
                                   'sleep_delay'],
                               interact=module.follow_by_list['interact'])
    else:
        print('---> Following by list not enabled')

    if module.follow_user_followers[
        'enabled'] == True and type != 'only_unfollow':
        print('---> Starting following by someone elses followers')
        session.follow_user_followers(
            module.follow_user_followers['followlist'],
            amount=module.follow_user_followers['amount'],
            randomize=module.follow_user_followers['randomize'],
            sleep_delay=module.follow_user_followers['sleep_delay'],
            interact=module.follow_user_followers['interact'])
    else:
        print('---> Following by user following not enabled')

    if module.follow_user_following[
        'enabled'] == True and type != 'only_unfollow':
        print('---> Starting following by someone elses following')
        session.follow_user_following(
            module.follow_user_following['followlist'],
            amount=module.follow_user_following['amount'],
            randomize=module.follow_user_following['randomize'],
            sleep_delay=module.follow_user_following['sleep_delay'],
    ...
    if module.follow_by_tags['enabled'] == True and type != 'only_unfollow':
        ...
        session.follow_by_tags(tags=module.follow_by_tags['tags'],
                               amount=module.follow_by_tags['amount'],
                               randomize=module.follow_by_tags['randomize'],
                               skip_top_posts=module.follow_by_tags[
                                   'skip_top_posts'],
    ...
    if module.follow_likers['enabled'] == True and type != 'only_unfollow':
        ...
        session.follow_likers(module.follow_likers['followlist'],
                              photos_grab_amount=module.follow_likers[
                                  'photos_grab_amount'],
                              follow_likers_per_photo=module.follow_likers[
                                  'follow_likers_per_photo'],
                              randomize=module.follow_likers['randomize'],
                              sleep_delay=module.follow_likers['sleep_delay'],
                              interact=module.follow_likers['interact'])
    else:
    ...
    if module.follow_commenters['enabled'] == True and type != 'only_unfollow':
        ...
        session.follow_commenters(module.follow_commenters['followlist'],
                                  amount=module.follow_commenters['amount'],
                                  daysold=module.follow_commenters['daysold'],
                                  max_pic=module.follow_commenters['max_pic'],
                                  sleep_delay=module.follow_commenters[
                                      'sleep_delay'],
                                  interact=module.follow_commenters[
                                      'interact'])
    else:
    ...
    if module.interact_by_users['enabled'] == True and type != 'only_unfollow':
        ...
        session.interact_by_users(module.interact_by_users['interactlist'],
                                  amount=module.interact_by_users['amount'],
                                  randomize=module.interact_by_users[
                                      'randomize'],
                                  media=module.interact_by_users['media'])
    else:
        print('---> Interacting by users not enabled')

    if module.interact_user_following[
        'enabled'] == True and type != 'only_unfollow':
        print('---> Starting interacting by elses following')
        session.interact_user_following(
            module.interact_user_following['interactlist'],
            amount=module.interact_user_following['amount'],
            randomize=module.interact_user_following['randomize'])
    else:
        print('---> Interacting by elses following not enabled')

    if module.interact_user_followers[
        'enabled'] == True and type != 'only_unfollow':
        print('---> Starting interacting by elses following')
        session.interact_user_followers(
            module.interact_user_followers['interactlist'],
            amount=module.interact_user_followers['amount'],
    ...
    if module.interact_by_URL['enabled'] == True and type != 'only_unfollow':
        ...
        session.interact_by_URL(urls=module.interact_by_URL['urls'],
                                randomize=module.interact_by_URL['randomize'],
    ...
    if module.comment_by_locations[
        'enabled'] == True and type != 'only_unfollow':
        print('---> Starting commenting by locations')
        session.comment_by_locations(
            locations=module.comment_by_locations['locations'],
            amount=module.comment_by_locations['amount'],
            skip_top_posts=module.comment_by_locations['skip_top_posts'],
            media=module.comment_by_locations['media'])
    else:
    ...
    if module.like_by_tags['enabled'] == True and type != 'only_unfollow':
        ...
        session.like_by_tags(tags=module.like_by_tags['tags'],
                             amount=module.like_by_tags['amount'],
                             randomize=module.like_by_tags['randomize'],
    ...
    if module.like_by_feed['enabled'] == True and type != 'only_unfollow':
        ...
        session.like_by_feed(amount=module.like_by_feed['amount'],
                             unfollow=module.like_by_feed['unfollow'],
                             sleep_delay=module.like_by_feed['sleep_delay'],
    ...
    if module.like_by_locations['enabled'] == True and type != 'only_unfollow':
        ...
        session.like_by_locations(
            locations=module.like_by_locations['locations'],
            amount=module.like_by_locations['amount'],
            skip_top_posts=module.like_by_locations['skip_top_posts'],
            media=module.like_by_locations['media'])
    else:
        print('---> Liking by locations not enabled')

    ####
    if module.unfollow_users['enabled'] == True and (
            type == 'unfollow' or type == 'only_unfollow') and \
            module.unfollow_users['method'] == 'instapyfollowed':
        print('---> Starting unfollowing with method: instapyfollowed')
        session.unfollow_users(amount=module.unfollow_users['amount'],
                               sleep_delay=module.unfollow_users[
                                   'sleep_delay'],
                               style=module.unfollow_users['style'],
                               unfollow_after=module.unfollow_users[
                                   'unfollow_after'], InstapyFollowed=(
            True, module.unfollow_users['InstaPyFollowed']))
    else:
        print('---> Unfollowing with method: instapyfollowed not enabled')

    if module.unfollow_users['enabled'] == True and (
            type == 'unfollow' or type == 'only_unfollow') and \
            module.unfollow_users['method'] == 'all':
        ...
        session.unfollow_users(amount=module.unfollow_users['amount'],
                               ...
                               style=module.unfollow_users['style'],
                               unfollow_after=module.unfollow_users[
                                   'unfollow_after'], allFollowing=True)
    else:
