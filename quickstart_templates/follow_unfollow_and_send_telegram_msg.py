    ...
    with smart_run(session):
        ...
        while counter < 5:
            ...
            try:
                ...
                session.unfollow_users(amount=25, allFollowing=True, style="LIFO", unfollow_after=3 * 60 * 60,
                                       sleep_delay=450)
...
def xunfollow():
    requests.get(
        "https://api.telegram.org/******/sendMessage?chat_id=*****&text='InstaPy Unfollower WEDNESDAY Started @ {}'"
        .format(datetime.now().strftime("%H:%M:%S")))
    ...
    with smart_run(session):
        try:
            ...
            # actions
            session.unfollow_users(amount=1000, allFollowing=True, style="RANDOM", unfollow_after=3 * 60 * 60,
                                   sleep_delay=450)
